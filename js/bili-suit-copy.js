...
if(user_equip) {
    ...
    else obj.data["user_equip"] = user_equip[parseInt($prefs.valueForKey("bili_skin_num") || 1) - 1];
}
if(load_equip) {
    var load_equip = JSON.parse(load_equip);
    if (load_equip.length === 1) obj.data["load_equip"] = load_equip[0];
    else obj.data["load_equip"] = load_equip[parseInt($prefs.valueForKey("bili_load_num") || 1) - 1];
}
