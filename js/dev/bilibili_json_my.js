...
const scriptName = "BiliBili", storyAidKey = "bilibili_story_aid", blackKey = "bilibili_feed_black";
let magicJS = MagicJS("BiliBili", "INFO"), blacklist = [];
magicJS.read(blackKey) ? blacklist = magicJS.read(blackKey).split(";") : (magicJS.write(blackKey, ""), blacklist = "".split(";")), (() => {
    let t = null;
    if (magicJS.isResponse) switch (!0) {
        case/^https:\/\/app\.bilibili\.com\/x\/v2\/feed\/index\?/.test(magicJS.request.url):
            try {
                let e = JSON.parse(magicJS.response.body), i = [];
                for (let a of e.data.items) if (a.hasOwnProperty("banner_item")) {
                    let s = [];
                    for (let r of a.banner_item) if ("ad" !== r.type) {
                        if ("static" === r.type) continue; else r.static_banner && !0 != r.static_banner.is_ad_loc && s.push(r)
                    }
                    s.length >= 1 && (a.banner_item = s, i.push(a))
                } else a.hasOwnProperty("ad_info") || blacklist.includes(a.args.up_name) || -1 !== a.card_goto.indexOf("ad") || "small_cover_v2" !== a.card_type && "large_cover_v1" !== a.card_type && "large_cover_single_v9" !== a.card_type || i.push(a);
        ...
        case/^https?:\/\/app\.bilibili\.com\/x\/v2\/feed\/index\/story\?/.test(magicJS.request.url):
            try {
                let l = JSON.parse(magicJS.response.body), o = [];
                for (let d of l.data.items) d.hasOwnProperty("ad_info") || -1 !== d.card_goto.indexOf("ad") || o.push(d);
        ...
        case/^https?:\/\/app\.bilibili\.com\/x\/resource\/show\/tab/.test(magicJS.request.url):
            try {
                let g = new Set([39, 40, 41, 774, 857, 545, 151, 442, 99, 100, 101, 554, 556]), b = new Set([176, 107]),
                ...
                if (f || (f = "246834163"), h.data.top) {
                    let S = h.data.top.filter(t => ((222 === t.id || 107 === t.id) && (t.uri = `bilibili://story/${f}`, t.icon = "https://raw.githubusercontent.com/blackmatrix7/ios_rule_script/master/script/bilibili/bilibili_icon.png", t.tab_id = "Story_Top", t.name = "Story"), b.has(t.id)));
                    h.data.top = S
                }
                if (h.data.bottom) {
        ...
        case/^https?:\/\/app\.bilibili\.com\/x\/v2\/account\/mine/.test(magicJS.request.url):
            try {
                let _ = JSON.parse(magicJS.response.body),
                    $ = new Set([396, 397, 398, 399, 402, 404, 407, 410, 425, 426, 427, 428, 430, 432, 433, 434, 494, 495, 496, 497, 500, 501]);
                _.data.sections_v2.forEach((t, e) => {
                    t.items.forEach(t => {
                        622 === t.id && (t.title = "会员购", t.uri = "bilibili://mall/home")
                    });
                    let i = t.items.filter(t => $.has(t.id));
                    _.data.sections_v2[e].button = {}, delete _.data.sections_v2[e].be_up_title, delete _.data.sections_v2[e].tip_icon, delete _.data.sections_v2[e].tip_title;
                    for (let a = 0; a < _.data.sections_v2.length; a++) ("创作中心" == _.data.sections_v2[a].title || "創作中心" == _.data.sections_v2[a].title) && (delete _.data.sections_v2[a].title, delete _.data.sections_v2[a].type);
                    delete _.data.vip_section_v2, delete _.data.vip_section, _.data.sections_v2[e].items = i, _.data.hasOwnProperty("live_tip") && (_.data.live_tip = {}), _.data.hasOwnProperty("answer") && (_.data.answer = {}), _.data.vip_type = 2, _.data.vip.type = 2, _.data.vip.status = 1, _.data.vip.vip_pay_type = 1, _.data.vip.due_date = 4669824160
                }), t = JSON.stringify(_)
            } catch (k) {
        ...
        case/^https?:\/\/app\.bilibili\.com\/x\/resource\/top\/activity/.test(magicJS.request.url):
            try {
                ...
                E.data && (E.data.hash = "ddgksf2013", E.data.online.icon = ""), t = JSON.stringify(E)
            } catch (q) {
        ...
        case/^https?:\/\/app\.bilibili\.com\/x\/v2\/search\/square/.test(magicJS.request.url):
            try {
                ...
                O.data = {type: "history", title: "搜索历史", search_hotword_revision: 2}, t = JSON.stringify(O)
            } catch (P) {
        ...
        case/https?:\/\/app\.bilibili\.com\/x\/v2\/account\/myinfo\?/.test(magicJS.request.url):
            try {
                let W = JSON.parse(magicJS.response.body);
                W.data.vip.type = 2, W.data.vip.status = 1, W.data.vip.vip_pay_type = 1, W.data.vip.due_date = 4669824160, t = JSON.stringify(W)
            } catch (B) {
        ...
        case/^https?:\/\/api\.vc\.bilibili\.com\/dynamic_svr\/v1\/dynamic_svr\/dynamic_(history|new)\?/.test(magicJS.request.url):
            try {
                let A = JSON.parse(magicJS.response.body), F = [];
                A.data.cards.forEach(t => {
                    t.hasOwnProperty("display") && 0 >= t.card.indexOf("ad_ctx") && (t.desc.dynamic_id = t.desc.dynamic_id_str, t.desc.pre_dy_id = t.desc.pre_dy_id_str, t.desc.orig_dy_id = t.desc.orig_dy_id_str, t.desc.rid = t.desc.rid_str, F.push(t))
                }), A.data.cards = F, t = JSON.stringify(A)
            } catch (T) {
        ...
        case/^https?:\/\/app\.bilibili\.com\/x\/resource\/show\/skin\?/.test(magicJS.request.url):
            try {
                ...
                j && j.hasOwnProperty("data") && j.data.hasOwnProperty("common_equip") && j.data.common_equip.hasOwnProperty("package_url"), t = JSON.stringify(j)
            } catch (z) {
        ...
        case/^https:\/\/app\.bilibili\.com\/x\/v2\/splash\/list/.test(magicJS.request.url):
            try {
                ...
                if (C.data && C.data.list) for (let D of C.data.list) D.duration = 0, D.begin_time = 2240150400, D.end_time = 2240150400;
                t = JSON.stringify(C)
            } catch (G) {
