...
} else if (url.includes('/vip/v2/theme') && url.includes('op=gd')) {
    let obj = JSON.parse($response.body)
    obj.data.needBieds = null
    body = JSON.stringify(obj)
} else if (url.includes('/vip/enc/user/vip') && method == 'GET') {  // vip1.*enc
...
} else if (url.includes('Serv') && method == 'GET') {
    body = '{}'
} else {
