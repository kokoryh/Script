    ...
    switch (!0) {
        case/pgc\/season\/app\/related\/recommend\?/.test($request.url):
            try {
                ...
                t.result?.cards?.length > 0 && (t.result.cards = t.result.cards.filter(t => 2 != t.type)), body = JSON.stringify(t)
            } catch (i) {
                console.log("related recommend" + i)
            }
            break;
        case/^https?:\/\/app\.bilibili\.com\/x\/resource\/show\/skin\?/.test($request.url):
            try {
                let e = JSON.parse(body);
                delete e.data?.common_equip, body = JSON.stringify(e)
            } catch (a) {
                console.log("show skin" + a)
            }
            break;
        ...
        case/^https?:\/\/app\.bilibili\.com\/x\/resource\/show\/tab/.test($request.url):
            try {
                ...
                if (h.data?.tab && (h.data.tab = [{
                    ...
                    let f = h.data.bottom.filter(t => u.has(t.id));
                    h.data.bottom = f
                }
        ...
        case/^https?:\/\/app\.bilibili\.com\/x\/v2\/account\/mine/.test($request.url):
            try {
                let g = JSON.parse(body),
                    v = new Set([396, 397, 398, 399, 407, 410, 402, 404, 425, 426, 427, 428, 430, 432, 433, 434, 494, 495, 496, 497, 500, 501]);
                g.data.sections_v2.forEach((t, i) => {
                    let e = t.items.filter(t => v.has(t.id));
                    g.data.sections_v2[i].items = e, g.data.sections_v2[i].button = {}, delete g.data.sections_v2[i].be_up_title, delete g.data.sections_v2[i].tip_icon, delete g.data.sections_v2[i].tip_title, "创作中心" == g.data.sections_v2[i].title && (delete g.data.sections_v2[i].title, delete g.data.sections_v2[i].type)
                }), delete g.data.vip_section_v2, delete g.data.vip_section, g.data.hasOwnProperty("live_tip") && (g.data.live_tip = {}), g.data.hasOwnProperty("answer") && (g.data.answer = {}), g.data.vip.status || (g.data.vip_type = 2, g.data.vip.type = 2, g.data.vip.status = 1, g.data.vip.vip_pay_type = 1, g.data.vip.due_date = 466982416e4), body = JSON.stringify(g)
            } catch ($) {
        ...
        case/^https?:\/\/api\.live\.bilibili\.com\/xlive\/app-room\/v1\/index\/getInfoByRoom/.test($request.url):
            try {
                ...
                _.data.activity_banner_info = null, _.data?.shopping_info && (_.data.shopping_info = {is_show: 0}), _.data?.new_tab_info?.outer_list && _.data.new_tab_info.outer_list.length && (_.data.new_tab_info.outer_list = _.data.new_tab_info.outer_list.filter(t => 33 != t.biz_id)), body = JSON.stringify(_)
            } catch (x) {
        ...
        case/^https?:\/\/app\.bilibili\.com\/x\/resource\/top\/activity/.test($request.url):
            try {
                ...
                k.data && (k.data.hash = "ddgksf2013", k.data.online.icon = ""), body = JSON.stringify(k)
            } catch (w) {
        ...
        case/^https?:\/\/app\.bilibili\.com\/x\/v2\/search\/square/.test($request.url):
            try {
                ...
                W.data = {type: "history", title: "搜索历史", search_hotword_revision: 2}, body = JSON.stringify(W)
            } catch (j) {
        ...
        case/https?:\/\/app\.bilibili\.com\/x\/v2\/account\/myinfo\?/.test($request.url):
            try {
                ...
                q.data.vip.status || (q.data.vip.type = 2, q.data.vip.status = 1, q.data.vip.vip_pay_type = 1, q.data.vip.due_date = 466982416e4), body = JSON.stringify(q)
            } catch (E) {
        ...
        case/^https:\/\/app\.bilibili\.com\/x\/v2\/splash\/list/.test($request.url):
            try {
                ...
                if (I.data && I.data.list) for (let R of I.data.list) R.duration = 0, R.begin_time = 2240150400, R.end_time = 2240150400;
        ...
        case/^https:\/\/api\.live\.bilibili\.com\/xlive\/app-interface\/v2\/index\/feed/.test($request.url):
            try {
                let T = JSON.parse(body);
                T.data && T.data.card_list && (T.data.card_list = T.data.card_list.filter(t => "banner_v1" != t.card_type)), body = JSON.stringify(T)
            } catch (A) {
